...
export const getAnalysisFromCache = async (username: string): Promise<AnalysisResult | null> => {
  try {
    ...
    return {
      ...
      posts,
      engagementMetrics,
      timestamp: metricsData.created_at,
    };
  } catch (error) {
...
export const storeAnalysisResults = async (analysis: AnalysisResult): Promise<boolean> => {
  try {
    ...
    const { data: profileData, error: profileError } = await supabase
      ...
      }, { onConflict: 'username', returning: 'representation' });
...
export const getSearchHistory = async (
  ...
  try {
    // Check if search_history table exists first
    ...
    if (!tableExists) {
      console.warn('search_history table does not exist yet');
